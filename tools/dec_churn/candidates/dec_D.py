...
class ModeTransitionManager:
  ...
  def __init__(self, governor_enabled: bool = False):
    ...
    # CANDIDATE D: transition-rate governor. When more than GOV_BUDGET non-emergency transitions
    # have occurred in the trailing GOV_WINDOW frames, raise the change-threshold so further
    # marginal flips are damped until the rate falls. Disabled (governor_enabled=False) => the whole
    # block is inert and behavior is byte-identical to baseline (radarless passes False). The
    # governor ONLY ever raises the non-emergency change-threshold; the emergency branch returns
    # before any of this, so the governor can NEVER block, delay, or alter the emergency path.
    self.governor_enabled = governor_enabled
    self.GOV_WINDOW = 200     # frames (~10s at 20Hz)
    self.GOV_BUDGET = 4       # transitions allowed in-window before damping engages
    self.GOV_THRESHOLD = 0.85  # elevated change-threshold while over budget (vs default 0.6)
    self._frame_count = 0
    self._transition_frames = []  # absolute frame indices of recent non-emergency transitions

  def _governor_threshold(self, base_threshold: float) -> float:
    if not self.governor_enabled:
      return base_threshold
    # prune to trailing window
    cutoff = self._frame_count - self.GOV_WINDOW
    self._transition_frames = [f for f in self._transition_frames if f >= cutoff]
    if len(self._transition_frames) >= self.GOV_BUDGET:
      return max(base_threshold, self.GOV_THRESHOLD)
    return base_threshold
  ...
  def request_mode(self, mode: ModeType, confidence: float = 1.0, emergency: bool = False):
    ...
    # CANDIDATE D: only damps changes (mode != current); staying is untouched. Never reached for
    # emergency requests (they returned above).
    if mode != self.current_mode:
      confidence_threshold = self._governor_threshold(confidence_threshold)
    ...
    if self.mode_confidence[mode] > confidence_threshold:
      if mode != self.current_mode and self.transition_timeout == 0:
        ...
        if self.governor_enabled:
          self._transition_frames.append(self._frame_count)
...
class DynamicExperimentalController:
  def __init__(self, CP: structs.CarParams, mpc, params=None):
    ...
    # CANDIDATE D: enable the transition-rate governor for radar cars only; radarless passes False
    # so its ModeTransitionManager is byte-identical to baseline (A4 invariance).
    self._mode_manager = ModeTransitionManager(governor_enabled=not CP.radarUnavailable)
