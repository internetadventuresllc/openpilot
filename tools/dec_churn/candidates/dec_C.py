...
class ModeTransitionManager:
  ...
  def __init__(self, leave_blended_threshold: float = None):
    self.current_mode: ModeType = 'acc'
    self.mode_confidence = {'acc': 1.0, 'blended': 0.0}
    self.transition_timeout = 0
    self.min_mode_duration = 10
    # CANDIDATE C: asymmetric change-threshold. Switching blended -> acc requires HIGHER acc
    # confidence than the default 0.6, so brief acc requests (lead flicker, momentary slow_down
    # clear) cannot tear blended down. None => 0.6 (byte-identical), which radarless passes.
    self.leave_blended_threshold = leave_blended_threshold if leave_blended_threshold is not None else 0.6
  ...
  def request_mode(self, mode: ModeType, confidence: float = 1.0, emergency: bool = False):
    ...
    # CANDIDATE C: leaving blended for acc uses the elevated leave_blended_threshold; entering
    # blended keeps 0.6 (genuine braking not delayed); staying keeps 0.3. Emergency returned above.
    if mode == self.current_mode:
      confidence_threshold = 0.3  # Lower threshold for faster response
    elif self.current_mode == 'blended' and mode == 'acc':
      confidence_threshold = self.leave_blended_threshold
    else:
...
class DynamicExperimentalController:
  def __init__(self, CP: structs.CarParams, mpc, params=None):
    ...
    # CANDIDATE C: radar cars require 0.85 acc-confidence to leave blended; radarless passes None
    # (== 0.6, byte-identical to baseline) so radarless dynamics are unchanged (A4 invariance).
    leave_blended_threshold = None if CP.radarUnavailable else 0.85
    self._mode_manager = ModeTransitionManager(leave_blended_threshold=leave_blended_threshold)
