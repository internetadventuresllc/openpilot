...
class ModeTransitionManager:
  ...
  def __init__(self, leave_blended_dwell: int = None):
    self.current_mode: ModeType = 'acc'
    self.mode_confidence = {'acc': 1.0, 'blended': 0.0}
    self.transition_timeout = 0
    self.min_mode_duration = 10
    # CANDIDATE A: asymmetric per-mode dwell. Entering acc (== LEAVING blended) requires a longer
    # dwell than the symmetric default, so a brief urgency dip clearing slow_down for a few frames
    # cannot tear blended down and immediately re-enter it. None => symmetric min_mode_duration
    # (byte-identical to baseline), which radarless passes so its dynamics are unchanged.
    self.leave_blended_dwell = leave_blended_dwell if leave_blended_dwell is not None else self.min_mode_duration
  ...
  def request_mode(self, mode: ModeType, confidence: float = 1.0, emergency: bool = False):
    ...
    # CANDIDATE A: dwell required before THIS request can change the mode. Leaving blended for acc
    # uses the (>=) longer leave_blended_dwell; every other transition keeps min_mode_duration.
    # Emergency requests returned above, so the emergency path is byte-identical.
    required_dwell = self.leave_blended_dwell if (self.current_mode == 'blended' and mode == 'acc') else self.min_mode_duration
    if self.mode_duration < required_dwell and not override_holds:
      return
...
class DynamicExperimentalController:
  def __init__(self, CP: structs.CarParams, mpc, params=None):
    ...
    # CANDIDATE A: radar cars get the longer leave-blended dwell (24 frames == 1.2s); radarless cars
    # pass None so their ModeTransitionManager is byte-identical to baseline (A4 invariance).
    leave_blended_dwell = None if CP.radarUnavailable else 24
    self._mode_manager = ModeTransitionManager(leave_blended_dwell=leave_blended_dwell)
