...
class DynamicExperimentalController:
  def __init__(self, CP: structs.CarParams, mpc, params=None):
    ...
    # CANDIDATE B: urgency-source smoothing retune. Raise slow_down smoothing 0.70 -> 0.80 to damp
    # urgency oscillation around the slow_down threshold (the churn driver). Analytic step-response
    # cost: emergency detection (urgency>0.7) on a full 0->1 step moves from frame 3 to frame 4
    # (+1 frame == +0.05s); slow_down ENTER (>0.24) is unchanged at frame 0. Genuine emergency
    # braking is delayed by at most one model frame. RADARLESS keeps the baseline 0.70 (gated on
    # CP.radarUnavailable) so radarless dynamics are byte-identical to baseline (A4 invariance).
    slow_down_smoothing = 0.7 if CP.radarUnavailable else 0.8
    self._slow_down_filter = SmoothKalmanFilter(
      measurement_noise=0.1,
      process_noise=0.1,
      alpha=1.05,
      smoothing_factor=slow_down_smoothing
    )
